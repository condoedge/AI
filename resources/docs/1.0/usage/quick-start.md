...
1. **`nodeableConfig()` method** - In your model (highest priority, full override)
2. **`config/entities.php`** - Generated by `php artisan ai:discover` (required)

**Important:** You must run `php artisan ai:discover` to generate configuration. There is no runtime auto-discovery.

**Recommendation:**
- Most projects: Use `config/entities.php` (simple, centralized, version-controlled)
- Complex logic: Use `nodeableConfig()` for models needing dynamic configuration
