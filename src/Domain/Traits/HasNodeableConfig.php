...
trait HasNodeableConfig
{
    ...
    /**
     * Resolve configuration from config/entities.php
     *
     * Priority:
     * 1. nodeableConfig() method on model (explicit override)
     * 2. config/entities.php (generated by php artisan ai:discover)
     * 3. Model properties ($embedFields, etc.) merged on top
     *
     ...
     * @throws \RuntimeException If no configuration found
     */
    protected function resolveConfig(): array
    {
        ...
        // 1. Check for nodeableConfig() override (developer explicit override)
        if (method_exists($this, 'nodeableConfig')) {
            $override = $this->nodeableConfig();

            // If returns NodeableConfig builder, convert to array
            if ($override instanceof \Condoedge\Ai\Domain\ValueObjects\NodeableConfig) {
                $override = $override->toArray();
            }

            // If nodeableConfig() returns non-empty, use it directly
            if (!empty($override)) {
                $config = $override;
            }
        }

        // 2. If no override, check config/entities.php
        if (empty($config)) {
            $config = $this->readFromEntitiesConfig();
        }

        // 3. If still empty, throw - no runtime discovery
        if (empty($config)) {
            $modelClass = get_class($this);
            throw new \RuntimeException(
                "No configuration found for {$modelClass}. " .
                "Run 'php artisan ai:discover' to generate config/entities.php, " .
                "or implement nodeableConfig() method on your model."
            );
        }

        // 4. Merge model properties on top (allows quick tweaks)
        $modelProperties = $this->readModelProperties();
        if (!empty($modelProperties)) {
            $config = $this->mergeConfigDeep($config, $modelProperties);
        }
    ...
    /**
     * Read configuration from config/entities.php (warm cache)
     *
     * The entities.php file is generated by `php artisan ai:discover`
     * and serves as the primary configuration storage.
     *
     * @return array Configuration from entities.php or empty array
     */
    protected function readFromEntitiesConfig(): array
    {
